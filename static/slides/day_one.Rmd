...
#### 3. Blogdown - build your project website 
#### 4. xaringan, bookdown & Co
]
--
.pull-right[
...
#### .fancy[GOAL 3] 💪 Build your own data analysis project website   
#### .fancy[GOAL 4] 💪 Learn how to use R & Markdown to produce a multitude of outputs (books, presentations, interactive applications)
]
...
### Go to [menti.com](https://www.menti.com/) 🤸 and enter the code **6959 8626** 
...
## .fat[.fancy[Helpful resources (...that helped me creating these slides):]] 
...
for (i in 1:length(toc)) {
  if(substr(toc[i][1], 1, 2) == "# ") {
    toc[i] <- gsub("# ", "", toc[i], fixed = TRUE) %>% 
      gsub(".fancy", "", ., fixed = TRUE) %>% 
      gsub("[", "", ., fixed = TRUE) %>% 
      gsub("]", "", ., fixed = TRUE) %>% 
      gsub(".fat", "", ., fixed = TRUE) %>% 
      gsub(":", "", ., fixed = TRUE) %>% 
      gsub("<br>", "", ., fixed = TRUE) %>% 
      gsub(".salt", "", ., fixed = TRUE) %>% 
      gsub("#", "", ., fixed = TRUE)
...
row_outline <- which(doc == "# Detailed Outline")
row_body <- which(doc == "---")
...
<span style="background-color: #228B22; border-radius: 3px; padding: 4px; font-family: 'Source Code Pro', 'Lucida Console', Monaco, monospace;">&#91;visit my website](https&#58;//anneokk.netlify.app//)</span> becomes .out-t[[visit my website](https://anneokk.netlify.app/)]

You can also link text to an email address

<span style="background-color: #228B22; border-radius: 3px; padding: 4px; font-family: 'Source Code Pro', 'Lucida Console', Monaco, monospace;">&#91;email me](mailto&#58;a.k.kleine@rug.nl)</span> becomes .out-t[[email me](mailto:a.k.kleine@rug.nl)]
